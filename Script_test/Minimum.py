...
class Minimum():
    ...
    def getMiniGTO_df(self):
        ...
        mergeRefund_df = mergeRefund_df.dropna(subset=['Refund', 'Vendor','MMG'])
        self.MiniGTO_df = mergeRefund_df
        return self.MiniGTO_df 
          
    def getMinimum_df(self):
        MMG_df = self.getMMG_df().copy()
        ...
        for cost_center, group_df in minimum_df.groupby('Cost Center'):
            ...
            for i in range(n):
                ...
                if not match.empty:
                    ...
                    new_row['Transaction Date'] = matched_row['Transaction Date']
        ...
        result_df['Transaction Date'] = pd.to_datetime(result_df['Transaction Date'], format='%d/%m/%Y', errors='coerce')
