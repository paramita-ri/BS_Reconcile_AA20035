...
from GetInputReport import *
from GenNewReport import *
...
class ReconciliationApp:
    def __init__(self):
        ...
        self.GetReport_button = ttk.Button(self.button_frame, text="Get Report",
                                           command=self.start_get_report)
        self.GetReport_button.pack(pady=5)
    ...
    def run_reconciliation(self):
        try:
            ...
            getReport = GetReport(Pending_df, minimum_df, AAStar_df, GTO05_df, self)
            getReport.getReport()
    ...
    def start_get_report(self):
        ...
        self.running = True
        self.start_button.config(state=tk.DISABLED)
        self.GetReport_button.config(state=tk.DISABLED)
    ...
    def run_get_report(self):
        try:
            ...
            #step1 get file 
            getInputReport = GetInputReport(self)
            self.update_progress(20, "Get Report File...")
            Inputreport, MiniGTO_df = getInputReport.getFile()

            self.update_progress(50, "Get File Success...")

            genNewReport = GenNewReport(Inputreport, MiniGTO_df,self)
            genNewReport.getReport()
            self.update_progress(100,"Success Report Generation...")

        except Exception as e:
            # Print error to terminal
            import traceback
            print("\n" + "="*50)
            print("ERROR IN Generation Report PROCESS:")
            traceback.print_exc()
            print("="*50 + "\n")
            ...
            # Show error in GUI messagebox
            self.message_queue.put(('error', "Genarate Report Failed", str(e)))
        ...
        try:
            while True:
                ...
                if message[0] == 'progress':
                    ...
                    self.running = False
                    self.start_button.config(state=tk.NORMAL)
                    self.GetReport_button.config(state=tk.NORMAL)
                    if message[1]:
                        messagebox.showinfo("Success", message[1])
